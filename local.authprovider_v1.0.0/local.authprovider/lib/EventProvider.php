...
class EventProvider extends ProviderOAuth implements ProviderInterface
{
        ...
        foreach ($queryData as $key => $item)
        {
            ...
            if (!empty($item['query']['QUERY_URL']))
            {
                ...
                if ($this->isLocalUrl($url))
                {
                    ...
                    // Добавляем данные авторизации, если нужно
                    if (!empty($item['additional']['sendAuth']) && !empty($item['auth']))
                    {
                        $postData['auth'] = array(
                            'access_token' => $item['auth']['access_token'] ?? '',
                            'user_id'     => $item['auth']['user_id'] ?? 0,
                            'domain'     => $item['auth']['domain'] ?? '',
                            'member_id'    => $item['auth']['member_id'] ?? '',
                        );
                    }
    ...
    protected function isLocalUrl($url)
    {
        ...
        // ВАЖНО: Добавьте сюда ваш домен/хост, если бот на именованном сервере
        // Например:
        // if ($host === 'bot.mycompany.local') return true;
        // if ($host === 'internal-server.mycompany.ru') return true;

        // Если хотите ВСЕ события отправлять напрямую (самый простой вариант),
     ...
     * Логи будут в /local/modules/local.authprovider/debug.log
     */
